...
public class TSP {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            double distance = findPath(286, coordinates, allData);
    ...
    public static double findPath(int indexOfOrigin, double[][] coordinates, String[][] allDataOfTown) {
        boolean[] visited = new boolean[allDataOfTown.length];      // Check if current town is visited
        // Initialize the array
        for (boolean var : visited) {
            var = false;
        }
        double distance = 0.0;  // Accumulative distance
        int indexOfNext = indexOfOrigin - 1;    // Store the origin as "next"
        visited[indexOfOrigin - 1] = true;      // Set origin as visited
        ...
        for (int i = 0; i < allDataOfTown.length - 1; i++) {
            ...
            double[][] nearest = nearestTown(indexOfNext + 1, coordinates, allDataOfTown);      // Find routes ordered by distance
            double[] result = findFirstUnvisited(nearest, visited); // Find the nearest unvisited position
            indexOfNext = (int) result[0];  // Set next target position
            distanceOfNext = result[1];
            //System.out.println(allDataOfTown[indexOfNext][1] + ": " + round(distanceOfNext, 3) + " km");
            distance += distanceOfNext;     // Accumulate
            visited[indexOfNext] = true;    // Set current as visited
        }
    ...
    public static double[] findFirstUnvisited(double[][] sortedList, boolean[] visited) {
        ...
        for (int i = 0; i < sortedList.length; i++) {
            if (visited[(int) sortedList[i][0]] == false) {     // If find the first unvisited position, store and jump out the loop
    ...
    /**
     * Find the shortest route among all possibilities.
     *
     * @param coordinates   The array consists coordinates.
     * @param allDataOfTown The array stores index, town name, etc.
    ...
    public static double[] findShortestOverall(double[][] coordinates, String[][] allDataOfTown) {
        double[] finalResult = new double[2];               // Stores the final (minimum) answer
        double[] result = new double[allDataOfTown.length]; // All answers
        finalResult[0] = 0.0;
        finalResult[1] = Integer.MAX_VALUE;
        for (int i = 0; i < allDataOfTown.length; i++) {
            result[i] = findPath(i + 1, coordinates, allDataOfTown);    // Find the route
            System.out.println("Searching on " + (i + 1) + " (" + allDataOfTown[i][1] + "): " + round(result[i], 3) + " km");
        }
        for (int i = 0; i < result.length; i++) {   // Find the minimum in results
